...
import javaanpr.recognizer.RecognizedPattern;
import java.sql.*;
//import javaanpr.recognizer.RecognizedChar;
        ...
        for (Band b : carSnapshot.getBands()) { //doporucene 3
            ...
            for (Plate plate : b.getPlates()) {//doporucene 3
               ...
               // String parsedOutput = parser.parse(recognizedPlate, syntaxAnalysisMode);
                Parser.FinalPlate outputPlate =   parser.parseToPlate(recognizedPlate, syntaxAnalysisMode);
                ...
                System.out.println("Cost: " + outputPlate.getCost() +" "
                        + " Text identified: "+outputPlate.getPlate());
                if(outputPlate.getCost()>150)                    
                {                     
                    /////Start of Service
//                    String url = "http://localhost:8080/EmployeeAccess/numplate";
//
//                    URL obj = new URL(url);
...
//                con.setRequestProperty("numberplate", outputPlate.getPlate());
...
//		System.out.println(response.toString());        
            ...
            ////Start of DB fetch  
                ...
                "jdbc:mysql://localhost:3306/access","username","password");  
                //here access is database name, root is username and password 
                List<String> numPlateList = new ArrayList<String>();
                
                PreparedStatement stmt=con.prepareStatement("select * from tbl_numplatedetail where number = ?"); 
                
                stmt.setString(1,outputPlate.getPlate());
                ResultSet rs=stmt.executeQuery();
                System.out.println( rs.first());
               
               if(!rs.first()) { System.out.println("Numplate not recognized");return "Numplate not recognized";}
                else{
                   System.out.println("Number Plate "+rs.getString(1)+" Owner Name"+rs.getString(2));
                
                    System.out.println(rs.getString(1)+rs.getString(2));
                //numPlateList.add(rs.getString(2));  
                  String ownerName =rs.getString(2);
                con.close();
            return outputPlate.getPlate() + "Details :"+ ownerName;
                    } 
                }
